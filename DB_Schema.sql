...
-- Tableau : loans
CREATE TABLE IF NOT EXISTS loans (loan_id INTEGER PRIMARY KEY AUTOINCREMENT, book_id INTEGER NOT NULL, user_id INTEGER NOT NULL, borrowed_at DATE DEFAULT (datetime('now')), due_at DATE, returned_at DATE, status TEXT NOT NULL DEFAULT 'ONGOING' CHECK (status IN ('ONGOING', 'RETURNED', 'OVERDUE')), number_of_book INTEGER NOT NULL, FOREIGN KEY (book_id) REFERENCES books (book_id) ON UPDATE CASCADE ON DELETE RESTRICT, FOREIGN KEY (user_id) REFERENCES users (user_id) ON UPDATE CASCADE ON DELETE CASCADE);
